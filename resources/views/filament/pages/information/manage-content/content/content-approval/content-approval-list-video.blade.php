        ...
        <div
            ...
            <div class="tab_reel">
                <div class="tab_panel_video">
                    <form method="get"action="{{ route('filament.admin.resources.information.content.content-appoval-lists.index-video') }}">
                        <div class="flex gap-4">
                            ...
                            <button type="submit"
                                class="px-4 py-2 bg-blue-500 text-white rounded-full button-search">{{ __('admin/content.admin_content_list.search') }}</button>
                        </div>
                        ...
                        <div class="flex items-center justify-between mt-4 mb-4">
                            <label for="search-time"
                                ...
                                {{$count}} {{ __('admin/content.admin_content_list.results') }}
                                @endif

                            </label>
                            <select id="search-time" name="search-time" class="font-semibold border text-base rounded-xl block w-auto">
                                <option @selected(request('search-time')=='all') value="all">{{trans('system.filter.recently')}}</option>
                                <option @selected(request('search-time')=='old') value="old">{{trans('system.filter.oldest')}}</option>
                            </select>
                        </div>
                    </form>
                    <div class="flex items-center justify-between mt-4 mb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        @forelse ($results as $item)
                            <div
                                class="relative flex flex-col gap-4 pb-2 dark:border dark:border-white rounded-xl justify-center w-full shadow">

                                <div
                                    class="absolute top-1 left-1 bg-gray-700 bg-opacity-75 text-white text-xs font-bold px-2 py-1 rounded-md z-10">
                                    {{ $item->status == 1 ? 'Approval' : 'Requested' }}
                                </div>
                                @php
                                    preg_match(
                                ...
                                @endphp
{{--                                <a--}}
{{--                                    href="{{ route('filament.admin.resources.information.content.content-appoval-lists.view', $item->id) }}">--}}
...
{{--                                    </button>--}}
{{--                                </a>--}}
                                <iframe src="{{getYoutubeEmbedUrl($item->video_url)}}" class="w-full h-64 rounded-t-xl" frameborder="0"></iframe>

                                <div class="flex flex-col gap-2 px-4">
                                    <a
                                        href="{{ route('filament.admin.resources.information.content.content-appoval-lists.view', $item->id) }}"><p class="text-[#201F36] dark:text-white font-semibold text-lg truncate-2-lines">
                                            {{ $item->title }}</p></a>
                                    <p class="flex gap-4 text-[#464559] dark:text-white text-sm flex justify-between">
                                        <span>{{ $item->created_at }}</span>
                                        <span>{{ __('admin/content.admin_content_list.author') }}:
                                            {{ $item->getAuthor($item->system, $item->created_by)->fullName }}</span>
                                    </p>
                                </div>
            ...
            <div class="bg-white p-6 md:p-8 rounded-xl shadow-xl max-w-lg w-full mx-4 md:mx-8 relative">
                ...
                <div class="grid gap-6 mb-6 ">
                    <div>
                        <label for="content_name"
                            class="block mb-2 text-sm font-medium text-gray-700">{{ __('admin/content.admin_content_list.content_name') }}
                            <span class="text-red-500">*</span>
                        </label>
                        <input type="text" id="content_name"wire:model="content_name"
                    ...
                    <div>
                        <label for="link_url"
                            class="block mb-2 text-sm font-medium text-gray-700">{{ __('admin/content.admin_content_list.link_url') }} <span class="text-red-500">*</span></label>
                        <input type="text" id="link_url" wire:model="link_url"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3"
                            placeholder="{{ __('admin/content.admin_content_list.content_url_placeholder') }} (Youtube link)"
                            required />
                    </div>
                </div>
                ...
                <div class="mt-4">
                    <label for="placeholder_content"
                        class="block mb-2 text-sm font-medium text-gray-700">{{ __('admin/content.admin_content_list.content_introduction') }} <span class="text-red-500">*</span></label>
                    <textarea wire:model="intro" id="placeholder_content" required
