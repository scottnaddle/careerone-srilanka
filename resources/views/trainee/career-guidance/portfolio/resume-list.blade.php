...
@section('content')
    <p class="text-2xl mt-4 text-[#464559] font-semibold dark:text-white">Resume (CV)</p>
        ...
        <div class="flex flex-col gap-4 md:gap-6">
            @forelse($resumes as $key => $resume)
                <div class="flex gap-2 shadow-custom-light dark:shadow-custom-dark py-2 rounded-xl items-center dark:border-white border border-gray-300 px-4">
                    <img src="{{asset('images/cv.png')}}" class="w-20" alt="Resume">
                    <div class="flex flex-col gap-2 w-full">
                        <a href="{{route('trainee.career-guidance.portfolio.preview-resume', ['pid' => $resume->id])}}" class="text-sm md:text-base text-[#201F36] dark:text-white font-semibold">My Resume</a>
                        <div class="flex justify-between w-full">
                            <span class="text-xs md:text-sm text-[#91919A] dark:text-white">Updated: {{date('Y-m-d H:i:s', strtotime($resume->created_at))}}</span>
                            <div class="flex justify-end gap-4">
                                ...
                                {{--                                    </svg>--}}
                                {{--                                     Export PDF </a>--}}
                                <a href="{{route('trainee.career-guidance.portfolio.preview-resume', ['cid' => base64_encode($resume->id)])}}" target="_blank" class="text-primary text-xs md:text-sm flex items-center gap-1 hover:underline"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                    </svg> View </a>
                                <a href="{{route('trainee.career-guidance.portfolio.get-upload-resume', ['id' => $resume->id])}}" class="text-primary text-xs md:text-sm flex items-center gap-1 hover:underline"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                    </svg>
                                    Edit</a>
                                <button data-modal-target="delete-modal" data-modal-toggle="delete-modal" data-resume-id="{{$resume->id}}" class="text-red-600 text-xs md:text-sm flex items-center gap-1 btn-delete"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4">
            ...
            @empty
                <div class="flex flex-col gap-4 justify-center items-center">
                    ...
                    <p class="dark:text-white">You haven't had any Resume (CV)!</p>
                    <div class="flex flex-col lg:flex-row lg:justify-between gap-4">
                        <a href="{{route('trainee.career-guidance.portfolio.get-upload-resume')}}" class="text-white w-full xl:w-fit bg-[#4984F6] hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 rounded-full
 px-3 py-3 md:px-12 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 md:text-xl font-medium">Upload new Resume (CV)
                        </a>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function () {
            $(".btn-delete").click(function (e) {
                ...
                let id = $(this).attr('data-resume-id');
                let url = '/trainee/career-guidance/portfolio/delete-resume?id='+id;
