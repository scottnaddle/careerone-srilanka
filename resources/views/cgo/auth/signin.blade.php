...
@push('head')
    @include('auth.partials.firebase-messaging')
@endpush
    ...
    <div class="flex flex-col gap-2.5 w-full max-w-xl bg-white px-4 md:px-8 py-6 rounded-xl dark:bg-[#1E1E1E]">
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="grid grid-cols-1 gap-6 mt-4">
                <!-- Left column: Icon and greeting (same as Sign Up) -->
                <div class="flex flex-col items-center justify-center gap-4">
                    <img src="/images/cgo-icon.svg" alt="CGO portal icon">
                    <h1 class="text-2xl font-semibold text-gray-900 dark:text-white text-center">{{trans('auth.welcome_back')}}</h1>
                    <p class="text-primary font-semibold text-center text-base">{{trans('auth.Sign in to your CGO account.')}}</p>
                </div>

                <!-- Right column: Login form -->
                <div class="w-full flex flex-col items-center justify-center gap-6 md:col-span-2">

                    <form class="w-full flex flex-col gap-4 leading-5" action="{{ route('cgo.auth.postLogin') }}" method="POST" autocomplete="off">
                        @csrf

{{--                        <div class="text-center mb-2">--}}
{{--                            <p class="text-base font-normal text-gray-500 dark:text-gray-300">{{trans('auth.sign_in_to_continue')}}</p>--}}
{{--                        </div>--}}
                        ...
                        <div class="text-sm font-medium text-center text-gray-500 dark:text-gray-300 mt-2">
                            {{trans('auth.dont_have_account')}} <a href="{{ route('cgo.auth.register') }}"
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            // Script for show/hide password, in sync with Sign Up
