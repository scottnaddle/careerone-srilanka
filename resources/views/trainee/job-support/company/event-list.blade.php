...
@section('content')
    <div class="py-6">
        <x-breadcrumb :items="[
            ...
            ['label' => 'Company', 'url' => route('trainee.job-support.company.company-list')],
            [
                'label' => 'Company Detail',
                'url' => route('trainee.job-support.company.detail', ['id' => $company->id, 'slug' => $company->slug]),
            ],
            ['label' => 'Event list', 'url' => route('trainee.job-support.company.company-list')],
        ]" />
    </div>
    <div class="flex flex-col gap-5 bg-white dark:bg-[#1E1E1E] rounded-xl px-4 py-7 mb-6">
        <div class="flex gap-4 justify-center items-center">

            <form
                action="{{ route('trainee.job-support.company.event-list', ['id' => $company->id, 'slug' => $company->slug]) }}"
                method="GET" class="mx-auto flex gap-4 justify-center items-center w-full">
                <div class="relative w-full">
                    ...
                    <input type="search" id="default-search" name="title" value="{{ request()->query('title') }}"
        ...
        <div class="flex w-full gap-4 items-end justify-end">
            <select id="event_type" name="event_type"
                ...
                @foreach ($event_types as $event_type)
                    <option value="{{ $event_type->id }}" @selected(request()->input('event_type') == $event_type->id)>
                        {{ $event_type->code_name_en }}</option>
                @endforeach
            </select>
            <select id="sort_by" name="sort_by"
                ...
                <option value="recently" @selected(request()->input('sort_by') == 'recently')>Recently</option>
                <option value="oldest" @selected(request()->input('sort_by') == 'oldest')>Oldest</option>
            </select>
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                @forelse($events as $event)
                    <div
                        class=" flex flex-col gap-4 border-1 border-[#F8F8F8] md:p-4 bg-white dark:bg-[#1E1E1E] rounded-xl shadow-custom-light dark:shadow-custom-dark">
                        <div class="h-48 items-center flex justify-center">
                            @if (file_exists($event->thumbnail))
                                <img src="{{ $event->thumbnail }}" class=" rounded-xl object-cover h-full lg:h-48"
                                    alt="Event thumbnail">
                            @else
                                <img src="{{ asset('images/post.png') }}" class=" rounded-xl object-cover h-full lg:h-48"
                                    alt="Event thumbnail">
                            @endif
                        </div>
                        <div class="flex flex-col gap-3 w-full justify-between">
                            <div class="flex flex-col gap-3">
                                <a href="{{ route('trainee.job-support.company.event-detail', ['company' => $company->id, 'id' => $event->id, 'slug' => $event->slug]) }}"
                                    class="text-[#464559] dark:text-white text-xl font-semibold hover:text-primary dark:hover:text-primary">{{ \Str::limit($event->title, 80) }}</a>
    ...
    <script>
        ...
        $('#event_type').on('change', function() {
            if (url.searchParams.has('event_type')) {
                ...
                url.searchParams.append('event_type', this.value);
                url.searchParams.delete('page');
            }
