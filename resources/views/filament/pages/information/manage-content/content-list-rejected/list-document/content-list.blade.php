                    ...
                    <div id="content-request" class="content-page active">
                        <form method="get" action="{{ route('filament.admin.resources.content-list-rejecteds.index') }}">
                            <div class="flex gap-4">
                                ...
                                <option value="">{{ __('admin/content.admin_content_list.members') }}</option>
                                <option value="cgo" @selected(request()->query('member') == 'cgo')>
                                    {{ __('admin/content.admin_content_list.cgo') }}</option>
                                <option value="trainee" @selected(request()->query('member') == 'trainee')>
                                    {{ __('admin/content.admin_content_list.trainee') }}</option>
                                <option value="company" @selected(request()->query('member') == 'company')>
                                    {{ __('admin/content.admin_content_list.company') }}</option>
                                    <option value="admin" @selected(request()->query('member') == 'admin')>
                                        {{ __('admin/content.admin_content_list.admin') }}</option>
                            </select>

                                <button type="submit"
                                    class="flex-1 px-4 py-2 bg-blue-500 text-white rounded-full button-search">{{ __('admin/content.admin_content_list.search') }}</button>
                            </div>

                            <div class="flex items-center gap-4 flex justify-between mt-4 mb-4">
                                <label for="option" class="text-lg font-semibold text-[#706F81]">
                                    @if (!empty(request()->query()))
                                    {{$this->getTotal()}} {{ __('admin/company.result') }}
                                    @endif
                                </label>
                                <select id="search-time" name="search-time"
                                    class="font-semibold border text-base rounded-xl block">
                                    <option @selected(request()->query('search-time') == 'recently') value="recently">
                                        {{ __('admin/content.admin_content_list.recently') }}
                                    </option>
                                    <option @selected(request()->query('search-time') == 'oldset') value="oldset">
                                        {{ __('admin/content.admin_content_list.oldset') }}
                                    </option>
                                </select>
                            </div>
