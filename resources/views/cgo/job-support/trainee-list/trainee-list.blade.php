...
@section('title', 'CGO - Job support - Trainee List')
@push('css')
...
{{--        <p class="text-2xl text-[#464559] dark:text-white font-semibold">{{ trans('cgo.job_support.trainee_list.root') }}</p>--}}
        ...
        <div class="bg-white dark:bg-[#1E1E1E] rounded-xl px-4 py-5 flex flex-col gap-5">
            <form class="flex flex-col w-full gap-4" action="{{ route('cgo.job-support.trainee-list.list') }}" method="GET">
                <div class="flex gap-6 items-center">
                    <label for="simple-search" class="sr-only">{{ trans('cgo.job_support.trainee_list.search') }}</label>
                    <div class="relative w-full">
                        ...
                        <input type="text" name="search" id="simple-search" value="{{ request('search') }}"
                            ...
                            placeholder="{{ __('general.Trainee name or NIC') }}" />
                    </div>
                    <button type="submit"
                ...
                <div class="flex justify-between items-center">
                    <div class="flex flex-col">
                        ...
                        (request()->has('search') && request()->query('search') != '') ||
                            (request()->has('trainee_type') && request()->query('trainee_type') != 'all'))
                            <p class="text-[#706F81] text-lg font-semibold dark:text-white">{{ $trainees->total() }} {{ trans('cgo.filterResult') }}</p>
                        @else
                            <p></p>
                        @endif
                            <a href="{{route('cgo.job-support.trainee-list.trainees.export')}}" type="button" class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-xl">
                                 {{__('general.Download Trainee')}}
                            </a>
                    </div>
                    <div class="flex gap-4 items-center">
{{--                        <div class="flex items-center gap-1">--}}
{{--                            <input id="portfolio-check" type="checkbox" @checked(request()->get('has-portfolio') == 1) value="1" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-[#1E1E1E] dark:border-gray-600 ">--}}
{{--                            <span class="font-semibol dark:text-white text-[#706F81]">Portfolio</span>--}}
{{--                        </div>--}}
                        <select id="portfolio-check" name="portfolio-check"
                            ...
                            <option value="">{{ trans('trainee.my_page.portfolio') }}</option>
                            <option value="1" @selected(request()->get('has-portfolio') == 1)>{{ trans('trainee.my_page.has-portfolio') }}</option>
                            <option value="2" @selected(request()->get('has-portfolio') == 2)>{{ trans('trainee.my_page.no-portfolio') }}</option>
                        </select>
                        <select id="trainee_type" name="trainee_type"
                                class="bg-[#F8F8F8] font-semibold border border-gray-300 text-[#706F81] text-sm rounded-xl focus:border-primary block p-2.5 dark:bg-[#1E1E1E] dark:border-white dark:placeholder-white dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option value="all" @selected(request()->get('trainee_type') == strtolower('all'))>{{ trans('system.filter.all') }}</option>
                            <option value="keep" @selected(request()->get('trainee_type') == strtolower('keep'))>{{ trans('system.filter.keep') }}</option>
                            <option value="unkeep" @selected(request()->get('trainee_type') == strtolower('unkeep'))>{{ trans('system.filter.unkeep') }}</option>
                        </select>
                    </div>
            ...
            <div class="flex flex-col divide-y divide-inherit dark:divide-white lg:divide-y-0">
                @forelse ($trainees as $item)
                    <div class="flex flex-col gap-5 lg:flex-row justify-between lg:px-5 py-2 md:py-3 hover:bg-blue-100 dark:hover:bg-gray-700 rounded-xl">
                        <div class="flex gap-6 items-center">
                            @if (isset($item->profile_image))
                                <img class="w-16 h-16 rounded-full object-cover" src="{{ asset($item->profile_image) }}" alt="user photo">
                            @else
                            ...
                            <div class="flex flex-col gap-1">
                                <p class="flex gap-3">
                                    <button type="button"
                                        ...
                                        class="trainee-name-btn whitespace-nowrap ajax-call text-primary font-semibold hover:text-blue-700 dark:hover:text-white"
                                        data-full-name="{{ $item->fullName }}"
                                        data-limited-name="{{ \Str::limit($item->first_name . ' ' . $item->last_name, 13) }}">
                                        {{ $item->fullName }}
                                    </button>
...
{{--                                    {{ trans('cgo.job_support.trainee_list.updated') }} {{ \Carbon\Carbon::parse($item->updated_at)->diffForHumans() }}--}}
{{--                                </p>--}}
                            </div>
                        </div>
                        @if($item->open_to_work == 0)
                        <div class="flex gap-3 lg:gap:6 lg:items-center justify-end">
                            <a href="javascript:void(0)"
                                class="whitespace-nowrap px-3 md:px-4 lg:px-6 py-1 md:py-2 lg:py-3 font-medium text-primary bg-[#E8F0FF] rounded-full hover:bg-primary shadow-xs hover:text-white text-center text-sm md:text-base cursor-not-allowed" disabled="disabled" data-tooltip-target="not-open-to-ojt-{{$item->id}}" data-tooltip-style="light" >
                                {{ trans('system.action.ojt_match') }}
                            </a>
                            <div id="not-open-to-ojt-{{$item->id}}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 tooltip">
                                {{trans('general.not_allow_recommend_job')}}
                                <div class="tooltip-arrow" data-popper-arrow></div>
                            </div>
                            <a href="javascript:void(0)"
                                class="whitespace-nowrap px-3 md:px-4 lg:px-6 py-1 md:py-2 lg:py-3 font-medium text-white bg-primary rounded-full hover:bg-blue-700 shadow-xs text-center text-sm md:text-base cursor-not-allowed" disabled="disabled" data-tooltip-target="not-open-to-work-{{$item->id}}" data-tooltip-style="light" >
                                {{ trans('system.action.job_match') }}
                            </a>
                            <div id="not-open-to-work-{{$item->id}}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg shadow-sm opacity-0 tooltip">
                                {{trans('general.not_allow_recommend_job')}}
                                <div class="tooltip-arrow" data-popper-arrow></div>
                            </div>
                            <button onclick="handleKeepTrainee(this)" id="keep-trainee"
                                data-trainee-id="{{ $item->id }}"
                                data-keeper-id="{{ Auth::guard(activeGuard())->user()->id }}"
                                data-system="{{ activeGuard() }}">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" fill="none"
                                    class="{{ $item->isKeep() ? 'fill-primary' : '' }} size-6 md:size-7 lg:size-8">
                                    <path
                                        d="M6.66602 10.4C6.66602 8.15979 6.66602 7.03969 7.10199 6.18404C7.48548 5.43139 8.09741 4.81947 8.85005 4.43597C9.7057 4 10.8258 4 13.066 4H18.9327C21.1729 4 22.293 4 23.1486 4.43597C23.9013 4.81947 24.5132 5.43139 24.8967 6.18404C25.3327 7.03969 25.3327 8.15979 25.3327 10.4V28L15.9993 22.6667L6.66602 28V10.4Z"
                                        stroke="#4984F6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            </button>
                        </div>
                        @else
                            <div class="flex gap-3 lg:gap:6 lg:items-center justify-end">
                                <a href="{{ route('cgo.job-support.trainee-list.ojt-match', ['trainee' => $item]) }}"
                                   class="whitespace-nowrap px-3 md:px-4 lg:px-6 py-1 md:py-2 lg:py-3 font-medium text-primary bg-[#E8F0FF] rounded-full hover:bg-primary shadow-xs hover:text-white text-center text-sm md:text-base" >
                                    {{ trans('system.action.ojt_match') }}
                                </a>
                                <a href="{{ route('cgo.job-support.trainee-list.job-match', ['trainee' => $item]) }}"
                                   class="whitespace-nowrap px-3 md:px-4 lg:px-6 py-1 md:py-2 lg:py-3 font-medium text-white bg-primary rounded-full hover:bg-blue-700 shadow-xs text-center text-sm md:text-base">
                                    {{ trans('system.action.job_match') }}
                                </a>
                                <button onclick="handleKeepTrainee(this)" id="keep-trainee"
                                        data-trainee-id="{{ $item->id }}"
                ...
                @empty
                    <div class="flex flex-col justify-center items-center opacity-50">
                        <img src="{{asset('images/empty-box.png')}}" class="w-32" alt="Empty">
...
@push('js')
    ...
    <script>
        ...
        $('#portfolio-check').on('change', function () {
            ...
            if (selectedValue === 'all') {
                ...
                url.searchParams.set('has-portfolio', selectedValue);
                url.searchParams.delete('page');
            }
    ...
    <script>
        function handleKeepTrainee(button) {
            ...
            const keeperId = button.getAttribute('data-keeper-id');
            const system = button.getAttribute('data-system');
            $.ajax({
                url: '{{ route('cgo.job-support.trainee-list.keeptrainee') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function applyNameLimit() {
                ...
                buttons.forEach(function(button) {
                    ...
                    button.textContent = isMobile ? limitedName : fullName;
                });
            }
            ...
            $('.ajax-call').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#summary_training_block').html(response.trainee_user.sumary_training);
