    ...
    <div class="mb-6 flex flex-col">
        ...
        <div class="py-6">
            <x-breadcrumb :items="[
                ...
                ['label' => trans('cgo.job_support.ojt_list.list_matched.root'), 'url' => ''],
            ]" />
        </div>
                        ...
                        @foreach ($trainees as $trainee)
                            ...
                            <tr
                                ...
                                <td class="md:p-5">
                                    <div class="flex gap-6 items-center">
                                        ...
                                        <div class="flex flex-col gap-1 items-start">
                                            <p class="flex gap-3">
                                                <a href="{{ route('cgo.job-support.ojt-list.trainee-information', ['slug' => $ojt->slug, 'trainee' => $trainee]) }}"
                                                ...
                                                <span class="text-sm px-2.5 py-1 rounded shadow-xs text-white bg-primary">
                                                    ...
                                                    {{ getCGOName($trainee->id, $ojt->id, 'ojt') }}
                                                </span>
                                ...
                                <td class="px-4 py-3 text-sm text-[#706F81] dark:text-[#C9CCD4]">
                                    ...
                                    {{--                                        {{ $trainee->isMatchOJT($ojt->id) ? 'checked' : '' }} id="checked-checkbox" --}}
                                    {{--                                        type="checkbox" value="{{ $ojt->id }}" data-trainee-id="{{ $trainee->id }}" --}}
                                    {{--                                        data-matched-by="{{ Auth::guard(activeGuard())->user()->id }}" --}}
                                    {{--                                        data-system="{{ activeGuard() }}" --}}
                                    {{--                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-[#1E1E1E] dark:border-gray-600"> --}}

                                    {{-- <input onchange="handleCheckboxChange(this)"
                                           {{ $trainee->isMatchOJT($ojt->id) ? 'checked' : '' }} id="checked-checkbox"
                                           type="checkbox" value="{{ $ojt->id }}"
                                           data-trainee-id="{{ $trainee->id }}"
                                           data-matched-by="{{ $trainee->isMatchOJT($ojt->id) ? getCGOIdMatchedTraineeToOJT($trainee->id, $ojt->id) : Auth::guard(activeGuard())->user()->id }}"
                                           data-system="{{ activeGuard() }}"
