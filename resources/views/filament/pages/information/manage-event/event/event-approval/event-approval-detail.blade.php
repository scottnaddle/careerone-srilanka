...
{{--                            <img src="{{ Storage::url($event->thumbnail) }}" alt="Event image">--}}
...
{{--            </x-filament-panels::form>--}}
            ...
            <div class="mt-auto flex justify-end space-x-2 p-4">
                @if (
                    $contenData->status == \App\Enums\StatusEnumsManagement::PENDING_APPROVAL->value ||
                        $contenData->status == \App\Enums\StatusEnumsManagement::APPROVED->value)
                    <div class="mr-2">
                        <button type="button" wire:click="showApprovalModal"
                                class="bg-gray-300 text-gray-700 px-4 py-2 rounded-xl flex items-center justify-center text-sm font-semibold">
                            {{ __('Reject') }}
                        </button>
                    </div>
                @endif
