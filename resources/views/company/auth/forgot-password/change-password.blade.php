        ...
        <div class="flex flex-col gap-6">
            <a href="{{ route('company.auth.register') }}"
               class="text-[#404040] dark:text-white border-gray-200 gap-2 rounded-xl text-xl font-semibold w-fit text-center inline-flex items-center hover:text-primary dark:hover:text-primary">
                ...
                {{ trans('auth.company_sign_up') }}
            </a>

            <div class="flex flex-col gap-6 mt-4">
                <div class="flex flex-col items-center justify-center gap-4">
                    <h2 class="text-2xl font-semibold text-gray-900 dark:text-white text-center">{{trans('system.form.reset_password')}}</h2>
                    <p class="text-primary dark:text-white font-semibold text-center text-base">{{trans('auth.Create a new password for your Organisation Recruiter account.')}}</p>
                </div>

                <div class="w-full flex flex-col items-center justify-center gap-6 md:col-span-2">
                    <form class="w-full flex flex-col gap-4 leading-5" action="{{ route('company.auth.postResetPassword') }}" method="POST" autocomplete="off">
                        @csrf
                        ...
                        <div class="text-sm font-medium text-center text-gray-500 dark:text-gray-300 mt-2">
                            <a href="{{ route('company.auth.login') }}" class="text-primary hover:underline dark:text-blue-500">{{ trans('auth.Organisation Sign in') ?? 'Sign In' }}</a>
                        </div>
                    </form>
                </div>
            </div>
