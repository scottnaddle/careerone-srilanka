    ...
    <div class="mb-6 flex flex-col">
        ...
        <div class="bg-white dark:bg-[#1E1E1E] rounded-xl px-4 py-5 flex flex-col gap-5">
            ...
            <form class="flex flex-col w-full gap-4"
                action="{{ route('cgo.job-support.ojt-list.trainee-match', ['slug' => $ojt->slug]) }}" method="GET">
                ...
                @foreach ($trainees as $item)
                    <div
                        ...
                        <div
                            ...
                            {{-- <a href="{{ route('cgo.job-support.ojt-list.trainee-information', ['slug' => $ojt->slug, 'trainee' => $item]) }}"
                            ...
                            <input onchange="handleCheckboxChange(this)"
                                {{ $ojt->checkMatched($ojt->id, $item->id) ? 'checked' : '' }} id="checked-checkbox"
                                type="checkbox" value="{{ $ojt->id }}" data-trainee-id="{{ $item->id }}"
                                data-matched-by="{{ $ojt->checkMatched($ojt->id, $item->id) ? getCGOIdMatchedTraineeToOJT($item->id, $ojt->id) : Auth::guard(activeGuard())->user()->id }}"
                                data-system="{{ activeGuard() }}"
                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-[#1E1E1E] dark:border-gray-600 {{ $ojt->checkMatched($ojt->id, $item->id) && getCGOIdMatchedTraineeToOJT($item->id, $ojt->id) != Auth::guard(activeGuard())->user()->id ? 'cursor-not-allowed' : '' }}"
                                {{ $ojt->checkMatched($ojt->id, $item->id) && getCGOIdMatchedTraineeToOJT($item->id, $ojt->id) != Auth::guard(activeGuard())->user()->id ? 'disabled="disabled"' : '' }}>
