...
@section('title', 'Job support - OJT List - Trainee Information')
    ...
    <div class="my-6 flex flex-col gap-5">
        ...
        <div class="py-6">
            <x-breadcrumb :items="[
                ...
                [
                    'label' => trans('company.menu.job_support.ojt_list'),
                    'url' => route('company.job-support.ojt-list.list'),
                ],
                [
                    'label' => trans('cgo.job_support.ojt_list.trainee_match.root'),
                    'url' => route('company.job-support.ojt-list.candidate-list', ['slug' => $ojt->slug]),
                ],
        ...
        <input value="{{ $ojt->id }}" name="ojt_id" hidden />
                ...
                <div class="py-6 flex flex-col gap-9">
                    <div class="flex flex-col gap-4">
                        <p class="text-2xl text-[#4984F6] dark:text-white font-semibold">
                            {{ trans('cgo.job_support.ojt_list.trainee_match.trainee_information.basic_information') }}</p>
                        <div class="flex gap-6 flex-col lg:flex-row">
                            <div class="flex flex-col gap-4">
                                <div class="flex gap-4">
                                    ...
                                    <span class="text-sm text-[#464559] dark:text-white">{{ $trainee->fullName }}</span>
                                </div>
                                <div class="flex gap-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                        viewBox="0 0 20 20" fill="none">
                            ...
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="flex gap-4">
                                    ...
                                    <span class="text-sm text-[#464559] dark:text-white">{{ $trainee->contact_address }}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="flex gap-6 flex-col">
                        <div class="flex flex-col gap-4 w-full lg:w-1/2">
                            <div class="flex flex-col gap-4">
                                <p class="text-2xl text-[#4984F6] dark:text-white font-semibold">
                                    {{ trans('cgo.job_support.ojt_list.trainee_match.trainee_information.education') }}</p>
                            ...
                            <div class="flex flex-col gap-4">
                                <p class="text-2xl text-[#4984F6] dark:text-white font-semibold">
                                    {{ trans('cgo.job_support.ojt_list.trainee_match.trainee_information.certificate') }}
                                </p>
                                @if (isset($trainee->traineeInformation) && $trainee->traineeInformation->nvq_content != '')
                                        ...
                                        <div class="flex  flex-col gap-4">
                                            <div class="flex gap-4 items-baseline">
                                                ...
                                                <div class="flex flex-col gap-2">
                                                    <p>
                                                        <span
                                                            class="text-[#464559] text-xl font-semibold dark:text-white">{{ $item->QUALIFICATION_NAME }}
                                                        </span>
                                                        <span
                                ...
                                @if ($trainee->portfolio && $trainee->public_portfolio)
                                    <div class="flex flex-col gap-4">
                                        <div class="flex gap-4 items-center">
                                            ...
                                            <div class="flex flex-col gap-2">
                                                <p>
                                                    <a href="{{ route('trainee.career-guidance.portfolio.preview-portfolio', ['pid' => $trainee->portfolio->id]) }}"
                                                        target="_blank"
                                                        class="text-[#464559] text-xl font-semibold dark:text-white dark:text-white hover:text-primary dark:hover:text-primary flex items-center gap-1"><svg
                    ...
                    @php
                        $selectable = $traineeApply->firstWhere('selected', null);
                        $selected = $traineeApply->firstWhere('selected', '!=', null);
                    @endphp
                    ...
                    @if ($selectable || $selected)
                        @auth('company')
                            <div class="custom text-end">
                                @if ($selectable)
                                    <button id="btn-save" type="button" data-trainee-apply-id="{{ $selectable->id }}"
                                        class="py-2.5 px-4 text-white bg-primary hover:bg-blue-800 text-sm font-medium rounded-full">
                                        {{ __('company.Select') }}
                                    </button>
                                @elseif ($selected)
                                    @if (is_null($selected->employeed))
                                        <button id="btn-unselect" type="button" data-trainee-apply-id="{{ $selected->id }}"
                                            class="py-2.5 px-4 text-white bg-red-500 hover:bg-red-700 text-sm font-medium rounded-full">
                                            {{ __('company.Unselect') }}
                                        </button>
                                        <button id="btn-employee" type="button" data-trainee-apply-id="{{ $selected->id }}"
                                            class="py-2.5 px-4 text-white bg-primary hover:bg-blue-800 text-sm font-medium rounded-full">
                                            {{ __('admin/cgo_performance.approval') }}
                                        </button>
                                    @else
                                        {{-- <button id="btn-unapproval" type="button"
                                            data-trainee-apply-id="{{ $selected->id }}"
                                            class="py-2.5 px-4 text-white bg-red-500 hover:bg-red-700 text-sm font-medium rounded-full">
                                            {{ __('cgo.reject') }}
                                        </button> --}}
                                    @endif
    ...
    <script>
        $(document).ready(function() {
            $(document).on('click', '#btn-save', function() {
                const traineeApplyId = $(this).data('trainee-apply-id');
                const $buttonContainer = $(this).closest('.custom');
                toggleLoadingOverlay();
                $.ajax({
                    url: '{{ route('company.job-support.ojt-list.selected') }}',
                    method: 'POST',
                    data: {
                        trainee_apply_id: traineeApplyId,
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(response) {
                        if (response.status === 'success') {
                            showToast(response.message ?? 'Successfully!', 3000, response
                                .status ?? 'success');
                            const unselectBtn = `
                        <button id="btn-unselect" type="button"
                            data-trainee-apply-id="${traineeApplyId}"
                            class="py-2.5 px-4 text-white bg-red-500 hover:bg-red-700 text-sm font-medium rounded-full">
                            {{ __('company.Unselect') }}
                        </button>`;
                            const employeeBtn = `
                        <button id="btn-employee" type="button"
                            data-trainee-apply-id="${traineeApplyId}"
                            class="py-2.5 px-4 text-white bg-primary hover:bg-blue-800 text-sm font-medium rounded-full">
                            {{ __('admin/cgo_performance.approval') }}
                        </button>`;

                            $buttonContainer.html(unselectBtn + employeeBtn);
                        } else {
                            showToast(response.message ?? 'error!', 3000, response.status ??
                                'error');
                        }
                        toggleLoadingOverlay();
                    },
                    error: function(xhr) {
                        ...
                        showToast(res?.message ?? 'error!', 3000, res?.status ?? 'error');
                    }
                });
            });
            ...
            $(document).on('click', '#btn-unselect', function() {
                ...
                toggleLoadingOverlay();
                $.ajax({
                    url: '{{ route('company.job-support.ojt-list.unselected') }}',
                    method: 'POST',
                    data: {
                        trainee_apply_id: traineeApplyId,
                        _token: '{{ csrf_token() }}'
                    ...
                    success: function(response) {
                        ...
                        <button id="btn-save" type="button"
                            ...
                            $buttonContainer.html(selectBtn);
                        } else {
                            showToast(response.message ?? 'error!', 3000, response.status ??
                                'error');
                        }
                    ...
                    error: function(xhr) {
                        let res = xhr.responseJSON;
                        showToast(res?.message ?? 'Failed!', 3000, res?.status ?? 'error');
                    }
                });
            });
        });
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#btn-employee', function() {
                ...
                $.ajax({
                    url: '{{ route('company.job-support.ojt-list.employeed') }}',
                    method: 'POST',
                    data: {
                        trainee_apply_id: traineeApplyId,
                        _token: '{{ csrf_token() }}'
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'success') {
                            ...
                            `;
                            $buttonContainer.html(unapprovalBtn);
                        } else {
                            showToast(response.message ?? 'error!', 3000, response.status ??
                                'error');
                        }
            ...
            $(document).on('click', '#btn-unapproval', function() {
                const traineeApplyId = $(this).data('trainee-apply-id');
                const $buttonContainer = $(this).closest('.custom');
                toggleLoadingOverlay();

                $.ajax({
                    url: '{{ route('company.job-support.ojt-list.unemploy') }}',
                    method: 'POST',
                    data: {
                        trainee_apply_id: traineeApplyId,
                        _token: '{{ csrf_token() }}'
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 'success') {
                            ...
                            `;
                            $buttonContainer.html(unselectBtn);
                        } else {
                            showToast(response.message ?? 'error!', 3000, response.status ??
                                'error');
                        }
