        ...
        <div
            ...
            <div class="tab-wrapper">
                <ul
                    ...
                    <li class="flex-1 tab-switch">
                        <a href="{{ route('filament.admin.resources.content-list-rejecteds.index') }}"
                            class="text-sm sm:text-lg inline-block w-full p-4 rounded-xl focus:ring-4 focus:ring-blue-300 focus:outline-none ">
                            {{ __('admin/content.admin_content_list.documents') }}
                        </a>
                    </li>
                    <li class="flex-1 tab-active">
                        <a href="{{ route('filament.admin.resources.content-list-rejecteds.index-video')}}"
                            ...
                            aria-current="page">
                            {{ __('admin/content.admin_content_list.videos') }}
                        </a>
                    </li>
                </ul>
            </div>
            <div class="tab_reel">
                <div class="tab_panel_video">
                    <form method="get"action="{{ route('filament.admin.resources.content-list-rejecteds.index-video') }}">
                        ...
                        <div class="flex items-center justify-between mt-4 mb-4">
                            ...
                            <select id="search-time" name="search-time" class="font-semibold border text-base rounded-xl block ">
                                <option @selected(request()->query('search-time')=='recently') value="recently">{{ __('admin/content.admin_content_list.recent') }}</option>
                                <option @selected(request()->query('search-time')=='oldset') value="oldset">{{ __('admin/content.admin_content_list.old') }}</option>
                            </select>
                        </div>
                    </form>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        @foreach ($results as $item)
                            <div
                                class="relative flex flex-col gap-4 pb-2 dark:border dark:border-white rounded-xl justify-center w-fit">

                                <div
                                    class="absolute top-1 left-1 bg-gray-700 bg-opacity-75 text-white text-xs font-bold px-2 py-1 rounded-md z-10">
                                    {{ $item->status == 1 ? 'Non-approval' :($item->status == 0 ?'Requested':'Approval')}}
                                </div>
                                @php
                                    preg_match(
                                ...
                                @endphp
                                <a
                                    href="{{ route('filament.admin.resources.content-list-rejecteds.view', $item->id) }}">
                                ...
                                <div class="flex flex-col gap-2 px-4">
                                    ...
                                    <div class="flex justify-between items-center">
                                        <p class="text-[#706F81] text-xs dark:text-white flex items-center gap-2">
                                            ...
                                            <span>{{ __('admin/content.admin_content_list.play_time') }}: 07 : 26 :
                                                09</span>
                                        </p>
                                        <p class="text-[#706F81] text-xs dark:text-white">
                                            {{ __('admin/content.admin_content_list.author') }}:
                                            {{ $item->getAuthor($item->system, $item->created_by)->first_name }}</p>
                                    </div>
                                </div>
            ...
            <div class="bg-white p-6 md:p-8 rounded-xl shadow-xl max-w-lg w-full mx-4 md:mx-8 relative">
                ...
                <div class="mt-6 flex space-x-4">
                    ...
                    <button type="button" wire:click="approveItem"
                        class="bg-blue-600 px-5 py-2 rounded-full w-full hover:bg-blue-700 text-center">
