...
{{--    <p class="text-2xl text-[#464559] dark:text-white font-semibold"></p>--}}
    <div class="py-6">
        <x-breadcrumb :items="[
                ...
                ['label' => trans('cgo.job_support.job_list.candidate_list.title'), 'url' => '#'],
            ]" />
    </div>
    <div class="bg-white dark:bg-[#1E1E1E] rounded-xl px-4 py-5 flex flex-col gap-5">
        <div class="flex flex-col gap-4">
            @if( optional($job)->id )
                <a class="text-xl text-[#464559] dark:text-white font-semibold flex gap-4 items-center" href="{{ route('cgo.job-support.job-list.list') }}">
            ...
            <form class="flex flex-col w-full gap-4" action="{{ url()->current() }}" method="GET">
                <div class="flex gap-6 items-center">
                    ...
                    <button type="submit" class="px-6 lg:px-12 py-2 md:py-3 font-medium text-white bg-primary rounded-full hover:bg-blue-800  shadow-xs">
                        {{ trans('cgo.job_support.job_list.candidate_list.search') }}
                    </button>
                </div>
            ...
            <div class="flex flex-col dark:divide-white">
                ...
                <div class="flex gap-5 lg:flex-row justify-between lg:px-5 py-2 md:py-3 trainee-apply hover:bg-blue-100 dark:hover:bg-gray-700 rounded-xl">
                    <div class="flex gap-6 items-center">
                        @if(isset($apply->user->profile_image))
                            <img class="w-13 h-13 rounded-full min-w-16 max-w-16 min-h-16 object-cover p-1.5"  src="{{ asset($apply->user->profile_image) }}" alt="user photo">
                        @else
                        ...
                        <div class="flex flex-col gap-1">
                            ...
                            <p class="text-sm text-[#706F81] dark:text-white">
                                @if( optional($job)->id )
                                {{ $apply->apply_type == 'job_match' ? trans('company.matched_at') : trans('company.applied_at') }} {{ date('Y-m-d', strtotime($apply->apply_time)) }}
                                @else
                                {{ trans('cgo.job_support.job_list.candidate_list.filter.job_title') }}: {{ $apply->job->title }}
                                @endif
                    ...
                    @empty
                        <div class="flex flex-col gap-4 justify-center items-center p-4">
                            ...
                            <p class="dark:text-white">{{ trans('cgo.job_support.job_list.candidate_list.no_record') }}</p>
                        </div>
                @endforelse
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-[#1E1E1E] dark:border-white">
            ...
            <div class="flex items-center justify-between p-4 md:p-5 rounded-t-lg bg-primary dark:bg-[#383838] px-6 py-4">
                <h3 class="text-xl md:text-2xl lg:text-3xl font-semibold text-white">
                    {{ trans('cgo.job_support.job_list.candidate_list.modal.title') }}
                </h3>
...
<script type="module">
    ...
    $(document).ready(function() {
        ...
        function openCv($clickedElement) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Put data into modal
                    putDataIntoModal(response.trainee_user, response.trainee_information, response.trainee_certificates, response.trainee_resume, response.trainee_portfolio,response.trainee_portfolio_public, statusJob);
        ...
        function putDataIntoModal(trainee_user, trainee_information, trainee_certificates, trainee_resume,trainee_portfolio, trainee_portfolio_public, statusJob) {
