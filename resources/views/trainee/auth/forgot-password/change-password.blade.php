        ...
        <div class="flex flex-col gap-6">
            ...
            <a href="{{ route('trainee.auth.register') }}"
               class="text-[#404040] dark:text-white border-gray-200 gap-2 rounded-xl text-xl font-semibold w-fit text-center inline-flex items-center hover:text-primary dark:hover:text-primary">
                ...
                {{ trans('auth.trainee_sign_up') }}
            </a>

            <div class="flex flex-col gap-6 mt-4">
                <!-- Cột trái: Icon và lời chào -->
                <div class="flex flex-col items-center justify-center gap-4">
                    <img src="/images/trainee1.webp" alt="Trainee Icon">
                    <h2 class="text-2xl font-semibold text-gray-900 dark:text-white text-center">{{trans('auth.reset_password') ?? 'Reset Password'}}</h2>
                    <p class="text-primary dark:text-white font-semibold text-center text-base">{{trans('auth.Create a new password for your Trainee account.')}}</p>
                </div>

                <!-- Cột phải: Form Reset Password -->
                <div class="w-full flex flex-col items-center justify-center gap-6 md:col-span-2">
                    <form class="w-full flex flex-col gap-4 leading-5" action="{{ route('trainee.auth.postResetPassword') }}" method="POST" autocomplete="off">
                        @csrf
                        ...
                        <!-- Input: Email (xác nhận lại email) -->
                        <div>
                        ...
                        <div class="text-sm font-medium text-center text-gray-500 dark:text-gray-300 mt-2">
                            <a href="/trainee/cas/login" class="text-primary hover:underline dark:text-blue-500">{{trans('auth.trainee_sign_in') ?? 'Sign In'}}</a>
                        </div>
                    </form>
                </div>
            </div>
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            // Script cho việc Ẩn / Hiện mật khẩu
            ...
            // Script loại bỏ khoảng trắng khi nhập mật khẩu
