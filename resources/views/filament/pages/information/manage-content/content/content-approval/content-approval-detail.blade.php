                ...
                <div class="mt-auto flex justify-end space-x-2">
                    <div class="mr-2">
                        <button type="button" wire:click="showApprovalModal"
                            class="bg-gray-300 text-gray-700 px-4 py-2 rounded-xl font-semibold text-sm flex items-center space-x-2"
                            wire:loading.attr="disabled"
                            wire:target="showApprovalModal">
                            <span>{{ __('Reject') }}</span>
                            <svg wire:loading wire:target="showApprovalModal" class="animate-spin h-5 w-5 text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    ...
                    <div>
                        <button type="button" wire:click="handleApproval"
                            class="bg-blue-500 text-white px-4 py-2 rounded-xl text-sm font-semibold flex items-center space-x-2"
                            wire:loading.attr="disabled"
                            wire:target="handleApproval">
                            <span>{{ __('system.form.button.approve') }}</span>
                            <svg wire:loading wire:target="handleApproval" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 000 8v4a8 8 0 01-8-8z"></path>
                            </svg>
                        </button>
...
{{--                                <h2 class="text-medium font-semibold">{{ __('Content Approval') }}</h2>--}}
...
{{--                                class="block font-semibold text-center mb-4 text-2xl font-medium">{{ $this->detailid->fullName }}</span>--}}
                            ...
                            <div class="mt-4 flex">
                                <button type="button"  wire:target="closeModal" wire:click="closeModal"
                                  wire:loading.attr="disabled"
                                    class="bg-white text-gray-700 border mr-2 px-4 py-2 rounded rounded-xl w-1/2 text-center text-sm font-semibold">
                                    {{ __('Cancel') }}
                                    <svg wire:loading wire:target="closeModal" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 000 8v4a8 8 0 01-8-8z"></path>
                                    </svg>
                                </button>
                                <button type="button" wire:target="approveItem"  wire:click="approveItem"
                                   wire:loading.attr="disabled"
                                    class="bg-blue-500 text-white px-4 py-2 rounded rounded-xl w-1/2 text-center text-sm font-semibold">
                                    {{ __('Confirm') }}
                                    <svg wire:loading wire:target="approveItem" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 000 8v4a8 8 0 01-8-8z"></path>
                                    </svg>
                                </button>
