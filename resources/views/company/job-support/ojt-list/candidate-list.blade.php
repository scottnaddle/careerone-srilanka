...
@section('title', 'Job support - OJT List - Candidate List')
    ...
    <div class="mb-6 flex flex-col">
        ...
        <div class="bg-white dark:bg-[#1E1E1E] rounded-xl px-4 py-5 flex flex-col gap-5">
            <div class="flex flex-col gap-4">
                @if (optional($ojt)->id)
                    <a class="text-xl text-[#464559] dark:text-white font-semibold flex gap-4 items-center"
                        href="{{ route('company.job-support.ojt-list.list') }}">
                ...
                @else
                    <span class="text-xl text-[#464559] dark:text-white font-semibold flex gap-4 items-center">Candidate
                        list</span>
                @endif
                <form class="flex flex-col w-full gap-4" action="{{ url()->current() }}" method="GET">
                    <div class="flex gap-6 items-center">
                        ...
                        block w-full ps-10 p-2.5  dark:bg-[#1E1E1E] dark:border-white dark:placeholder-white dark:text-white"
                    ...
                    <div class="flex justify-between items-center flex-wrap">
                        @if (request()->has('search') && request()->query('search') != '')
                            <p class="text-[#706F81] text-lg font-semibold dark:text-white">{{ $applies->total() }} Results
                            </p>
                        @else
                            <p></p>
                        @endif
                ...
                <div class="flex flex-col gap-5 dark:divide-white">
                    ...
                    @php
                        ...
                        $groupInfo = $groupedApplies[$traineeId];
                    @endphp
                    <div class="flex gap-5 lg:flex-row justify-between lg:px-5 py-2 md:py-3 trainee-apply border-b border-[#F8F8F8] hover:bg-blue-100 dark:hover:bg-gray-700 rounded-xl">
                        <div class="flex gap-6 items-center">
                            @if ($apply->traineeMatched->profile_image)
                                <img class="w-16 h-16 rounded-full" src="{{ asset($apply->traineeMatched->profile_image) }}" alt="user photo">
                            @else
                                ...
                                <p class="flex gap-3 items-center flex-wrap">
                                    @if (optional($ojt)->id)
                                        <a href="{{ url()->full() }}{{ request()->all() != [] ? '&' : '?'}}trainee-apply-id={{ $apply->id }}"
                                            ...
                                            data-trainee-apply-id="{{ $apply->trainee_id }}"
                                            data-ojt-id="{{ $ojt->id }}"
                                            data-ojt-apply-id="{{ $apply->id }}"
                                            data-status-job="{{ $apply->ojt->status }}"
                                    ...
                                    @else
                                        <a href="{{ route('cgo.job-support.ojt-list.trainee-information', ['slug' => $ojt->slug, 'trainee' => $apply->traineeMatched]) }}"
                                           class="text-primary font-semibold hover:text-blue-700 dark:hover:text-white open-cv">
                                    ...
                                    @if ($groupInfo['matched'])
                                        ...
                                        <span class="text-sm px-2.5 py-1 rounded shadow-xs text-white bg-primary">
                                            {{ trans('cgo.job_support.ojt_list.list_matched.ojt_match') }}
                                            {{ getCGOName($traineeId, $groupInfo['matched']->ojt_id, 'ojt') }}
                                        </span>
                                    @endif
                                    ...
                                    @if ($groupInfo['apply'])
                                        <span class="text-[#464559] dark:text-white">|</span>
                                        <span class="text-sm px-2.5 py-1 rounded shadow-xs text-white bg-primary">
                                            {{ __('trainee.job_support.ojt_list.ojt_details.applied') }}
                                        </span>
                                    @endif
                                </p>
                                <p class="text-[#706F81] gap-3 dark:text-white flex flex-wrap">
                                    {!! getNewestTrainingInformationOfTrainee($traineeId) !!}
                                </p>
                                <p class="text-sm text-[#706F81] dark:text-white">
                                    @if (optional($ojt)->id)
                                        {{ trans('company.matched_at') }}:
                        ...
                        <div class="flex flex-col gap-6 items-end">
                            ...
                            <div class="flex gap-2">
                                ...
                                <span class="text-sm text-primary px-2 py-1 bg-[#E9F5FF] rounded-xl font-semibold {{ empty($apply->employeed) ? 'hidden' : '' }} employeed-trainee">Approved</span>
                            </div>
...
{{--                            </div> --}}
            ...
            <div class="flex justify-end px-2 py-4 pt-8 gap-2 md:gap-6 md:px-12" id="groud-btn-func">
                ...
                <button type="submit" class="py-2.5 px-4 text-white bg-primary hover:bg-blue-800 text-sm font-medium rounded-full block btn-employeed hidden">{{ __('admin/cgo_performance.approval') }}</button>
                ...
                <button type="submit" class="py-2.5 px-4 text-white bg-primary hover:bg-blue-800 text-sm font-medium rounded-full block btn-unselect hidden">{{ __('company.Unselect')}}</button>
            </div>
        </div>
    </div>
</div>
...
<script type="module">
    ...
    $(document).ready(function() {
        ...
        function openCv($clickedElement) {
            ...
            let ojt_id = $clickedElement.data('ojt-id');
            let ojt_trainee_apply_id = $clickedElement.data('ojt-apply-id');
            $.ajax({
                url: url,
                type: 'GET',
                success: function(response) {
                    ...
                    function handleScroll() {
                        ...
                        if (rect.top <= windowHeight && !hasEventFired) {
                            hasEventFired = true;
                                $.ajax({
                                    url: '{{ route('company.job-support.ojt-list.read') }}',
                                    type: 'PUT',
                                    data: {
                                        _token: '{{ csrf_token() }}',
                                        trainee_id: trainee_apply_id,
                                        ojt_id : ojt_id
                                    },
                                    success: function(response) {
                    ...
                    $('.btn-select').off('click').on('click', function() {
                        ...
                        $.ajax({
                            ...
                            data: {
                                _token: '{{ csrf_token() }}',
                                trainee_apply_id: ojt_trainee_apply_id,
                            },
                            success: function(response) {
                                if (typeof response.status !== 'undefined' && response.status !== 'error') {
                                    ...
                                    selectedSpan.removeClass('hidden');
                                     var rejectSpan = parentDiv.find('.rejected-cv');
                                    rejectSpan.addClass('hidden');
        ...
        function putDataIntoModal(trainee_user, trainee_information, trainee_certificates, trainee_resume,trainee_portfolio, trainee_portfolio_public, statusSelected, statusEmployeed, statusJob) {
            ...
            if (statusJob === statusProgress) {
                if (statusSelected) {
                    if (statusEmployeed) {
                        addButtonFunc('Approved', 'btn-employeed', true, 'bg-primary cursor-not-allowed opacity-50');
                    } else {
                        addButtonFunc('Approval', 'btn-employeed');
                        addButtonFunc('Unselect', 'btn-unselect');
                    }
                } else {
