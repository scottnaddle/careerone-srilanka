...
@php
    ...
    $activeMenu = collect($items)->some(fn ($item) =>
    method_exists($item, 'getSubItems') && !empty($item->getSubItems())
        ? collect($item->getSubItems())->some(fn ($subItem) => $subItem->isActive())
        : $item->isActive()
);
...
>
    @if ($label)
        ...
        >
            @if ($icon)
                <x-filament::icon
                    ...
                    @class([
                        ...
                        'stroke-stone-800' => ! $activeMenu,
                        'stroke-[#4984F6] ' => $activeMenu,
                    ])
                />
            @endif
            ...
            <span
                @class([
                    'text-[#4984F6]' => $activeMenu,
                    'text-gray-500 dark:text-gray-400' => ! $activeMenu,
                    'fi-sidebar-group-label flex-1 text-sm font-medium leading-6 '
                ])
            >
                {{ $label }}
            </span>
            ...
            @if ($collapsible)
                @php
                    $colorIconButton = $activeMenu ? 'text-[#4984F6] hovel:text-[#4984F6]' : 'text-gray-500 hover:text-[#4984F6]'
                @endphp
    ...
    >
        @foreach ($items as $item)
            @if(method_exists($item, 'getSubItems') && $subItems = $item->getSubItems())
                {{-- Begin --}}
                <div>
