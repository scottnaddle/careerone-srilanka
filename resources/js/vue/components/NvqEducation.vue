...
<!--                </button>-->
            ...
            <div v-if="showAddForm" class="p-4 border border-gray-300 rounded-lg mb-4">
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-white">Qualification Name</label>
                        <input v-model="newEducation.qualification_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-white">Level</label>
                        <input v-model="newEducation.level" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-white">Effective Date</label>
                        <input v-model="newEducation.effective_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                </div>
            ...
            <div class="flex flex-col gap-4">
                <div v-for="(education, index) in educations" :key="index" class="relative group  cursor-not-allowed">
                        ...
                        <div class="flex flex-col gap-2 w-full">
                            <div v-if="!editing[index]">
                                <p>
                                    ...
                                    <span class="text-[#706F81] dark:text-white"> ({{ education.level }})</span>
                                </p>
                                <p>
                                    <span class="text-[#91919A] dark:text-white"><span>{{ formatDate(education.effective_date) }}</span></span>
                                </p>
                            </div>
                            ...
                            <div v-else class="grid grid-cols-1 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-white">Qualification Name</label>
                                    <input v-model="editForms[index].qualification_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-white">Level</label>
                                    <input v-model="editForms[index].level" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-white">Effective Date</label>
                                    <input v-model="editForms[index].effective_date" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                                </div>
                            </div>
...
export default {
    ...
    methods: {
        ...
        removeEducation(index) {
            if (confirm('Are you sure you want to remove this NVQ qualification?')) {
