...
//        $enterprises = Enterprise::all();
    ...
    public function postRegister(RegisterCompanyRequest $request)
    {
        ...
        if ($company->save()) {
            // Restore signup form input from session
            $signupData = session('company_signup_input', []);
            return redirect()->route('company.auth.register')
                ->withInput($signupData)
                ->with('message', __('system.messages.company_registered'));
        }
