...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create(config('filament-menu-builder.tables.menu_icon'), function (Blueprint $table) {
            $table->id();
            $table->foreignIdFor(MenuItem::class, 'menu_item_id') 
                  ->constrained(config('filament-menu-builder.tables.menu_items')) 
                  ->cascadeOnDelete()
                  ->unique(); 
            $table->string('icon')->nullable();
            $table->timestamps();
    ...
    {
        Schema::dropIfExists(config('filament-menu-builder.tables.menu_icon'));
