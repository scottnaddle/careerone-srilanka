...
@section('content')
    <div class="flex flex-col gap-4 md:gap-6 my-10">
        <a href="{{route('trainee.career-guidance.portfolio.get-resume')}}" class="font-semibold text-xl hover:underline flex gap-2 items-center hover:text-primary dark:text-white dark:hover:text-primary">
            ...
            Back to list</a>
        <form action="{{route('trainee.career-guidance.portfolio.get-resume')}}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="bg-white dark:bg-[#1E1E1E] w-full p-4 md:p-6 rounded-xl shadow-custom-light dark:shadow-custom-dark">
                <div class="flex flex-col mb-3">
                    ...
                    <select type="text" name="type" id="title" class="w-full p-2.5 bg-white border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block dark:bg-[#1E1E1E] dark:placeholder-gray-400 dark:text-white" required>
                        ...
                        @forelse($tests as $test)
                            <option value="{{$test->type}}" {{(isset($test_result) && $test->type == $test_result->type) ? 'selected' : ''}}>{{$test->test_name}}</option>
                        @empty
                        @endforelse
                    </select>
                ...
                <div class="flex gap-3 items-center p-4 md:p-5 border-gray-200 rounded-b dark:border-gray-600">
                    <a href="{{route('trainee.career-guidance.career-test.list')}}" data-modal-hide="default-modal"
