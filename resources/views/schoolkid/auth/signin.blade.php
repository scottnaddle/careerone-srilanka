    ...
    <div class="max-w-2xl mx-auto w-screen py-24 flex flex-col h-full justify-start">
        <div
            ...
            <div class="flex flex-col gap-6">
                ...
                @endif
                <form class="leading-5" action="{{ route('schoolkid.auth.postLogin') }}" method="POST" autocomplete="off">
                    @csrf
                    <div class="mb-6">
                        <label for="email"
...
{{--                        <a href="/schoolkid/cas/login" class="w-full text-white bg-[#4984F6] hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-full--}}
{{--                        text-xl px-5 py-3 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 mb-4">Sign in using SSO</a>--}}
{{--                    </div>--}}
                    <div class="text-sm font-medium text-center text-gray-500 dark:text-gray-300 mt-4">
                        {{trans('auth.dont_have_account')}} <a href="{{ route('schoolkid.auth.register') }}"
