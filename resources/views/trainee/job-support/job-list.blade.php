...
@section('content')
    ...
    <div class="py-6">
        <x-breadcrumb :items="[
            ...
            [
                'label' => trans('trainee.menu.job_support.job_list'),
                'url' => route('trainee.job-support.company.company-list'),
            ],
        ]" />
    </div>
    <div class="mb-6 flex flex-col gap-5 bg-white dark:bg-[#1E1E1E] rounded-xl px-4 py-5">
        <div class="flex flex-col gap-4">
            <div class="space-y-5">
                @if (activeGuard() != '' && Auth::guard('trainee')->check())
                    <div class="grid grid-cols-3 bg-gray-50 rounded-lg shadow">
                        <button value="" name="status_job"
                            class="text-sm sm:text-lg p-4 focus:outline-none w-full text-center rounded-l-lg hover:font-bold @if (request()->get('status_job') == '') bg-primary text-white font-bold @else text-[#91919A] bg-[#F8F8F8] @endif ">{{ trans('trainee.job_support.job_list.all') }}</button>
                        <button value="applied" name="status_job"
                            class="text-sm sm:text-lg p-4 focus:outline-none w-full text-center hover:font-bold @if (request()->get('status_job') == 'applied') bg-primary text-white font-bold @else text-[#91919A] bg-[#F8F8F8] @endif ">{{ trans('trainee.job_support.job_list.applied') }}</button>
                        <button value="matched" name="status_job"
                            class="text-sm sm:text-lg p-4 focus:outline-none w-full text-center rounded-r-lg hover:font-bold  @if (request()->get('status_job') == 'matched') bg-primary text-white font-bold  @else text-[#91919A] bg-[#F8F8F8] @endif">{{ trans('trainee.job_support.job_list.matched') }}</button>
                    </div>
                @endif
            ...
            <div class="flex flex-col flex-col-reverse gap-2 md:flex-row md:justify-between items-center">
                @php
                    $queryParameters = request()->except(['page', 'sort_by', 'status_job']);
                ...
                <div class="mt-2 flex gap-4 float-right flex-wrap w-full md:flex-row md:justify-end">
                    ...
                    <button value="all" name="sector" id="sector-modal-open" type="button"
                        ...
                        <span>
                            @php
                                $sectorText = trans('trainee.job_support.company.filter.sector');
                    ...
                    <div class="child2 flex flex-col justify-around relative">
                        @if (activeGuard() != '' && Auth::guard('trainee')->check())
                            <div class="flex gap-2 justify-between">
                                <a href="{{ route('trainee.job-support.job-list.job-detail', ['job_id' => $item->id, 'slug' => $item->slug]) }}"
...
{{--                                @if (!empty($item->application_endtime))--}}
{{--                                    <span>--}}
{{--                                    {{ html_entity_decode('&#8226;') }}--}}
{{--                                    </span>--}}
{{--                                        <span--}}
{{--                                            class="{{ \Carbon\Carbon::parse($item->application_endtime)->isPast() ? 'text-red-600' : '' }}">--}}
{{--                                        {{ trans('admin/dashboard.job.application_end_date') }}:--}}
{{--                                        {{ \Carbon\Carbon::parse($item->application_endtime)->format('Y-m-d') }}--}}
{{--                                    </span>--}}
{{--                                @endif--}}
{{--                            </span>--}}
...
{{--                            @endif--}}
                        ...
                        @php
                            $statusJob = request()->query('status_job');
                        @endphp
                        @if (!empty($statusJob) && $statusJob == 'matched')
                            @php
                                $matched = \App\Models\TraineeMatch::where('job_id', $item->id)
                            ...
                            {{ \App\Enums\TypeTraineeApply::getNameByKey('job_match') }}
                                {{ getCGOName($matched->trainee_id, $matched->job_id) }} -
                        ...
                        @if (activeGuard() != '' && Auth::guard(activeGuard())->check() && activeGuard() == 'trainee')
                            <div class="flex gap-1">
                                @php
                                    $statusJob = request()->query('status_job');
                                @endphp
                                @if (!empty($statusJob) && $statusJob == 'applied')
                                    @php
                                        $apply = \App\Models\TraineeApply::where('job_id', $item->id)
                                            ->where('trainee_id', Auth::guard('trainee')->user()->id)
                                            ->first();
                                    @endphp
                                    <div class="flex gap-1">
                                        ...
                                        <div class="flex gap-2">
                                            <span
                                                class="text-sm text-primary px-2 py-1 bg-[#E9F5FF] rounded-xl font-semibold flex flex-col justify-center {{ empty($apply->selected) || !empty($apply->employeed) ? 'hidden' : '' }} selected-cv">Selected</span>
                                            <span
                                                class="text-sm text-primary px-2 py-1 bg-[#E9F5FF] rounded-xl font-semibold flex flex-col justify-center  {{ empty($apply->employeed) ? 'hidden' : '' }} employeed-trainee">Employed</span>
                                        </div>
                                    </div>
                                @endif

                                <button onclick="handleKeepTrainee(this)" id="keep-trainee"
                                    data-job-id="{{ $item->id }}" title="Bookmark job"
                                    data-trainee-id="{{ Auth::guard(activeGuard())->user()->id }}">

                                    <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21"
                                        viewBox="0 0 15 18"
                                        class="{{ $item->isMarkByTrainee(Auth::guard(activeGuard())->user()->id) ? 'fill-primary' : '' }} size-6 md:size-7 lg:size-8 cursor-pointer"
                                        fill="none">
                                        <path
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            let url = new URL(window.location.href);
            $('button[name=status_job]').on('click', function() {
                url.searchParams.set('status_job', $(this).val());
                url.searchParams.delete('page');
                window.location.href = url.href;
            });
